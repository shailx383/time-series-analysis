...
def _round_dt(dt, delta):
    '''
        rounds the datetime to the nearest specified timedelta

        Args:
            - dt (datetime): datetime to be rounded
            - delta (timedelta): timedelta which datetime must be rounded to
...
def impute_time_axis(time_axis: pd.Series, freq: timedelta = timedelta(days=1)):
    ...
    while idx < len(time_list):
        if pd.isnull(time_list[idx]):
            ...
            idx += 1
    seconds_intervals = [np.linspace(interval["start"].timestamp(
    ), interval["end"].timestamp(), interval["nulls"]) for interval in null_intervals]
    x = []
    for i in seconds_intervals:
        for j in i[1:-1]:
            x.append(j)
    final_fill_values = [pd.to_datetime(
        _round_dt(datetime.utcfromtimestamp(i), freq)) for i in x]
    column = []
    idx = 0
